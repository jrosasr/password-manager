  ...
  return (
    <div>
      ...
      {dataSidebarElements.map(({ title, icon: Icon, children }) => (
        <Accordion key={title} type="single" collapsible className="px-2 w-full">
      ...
      <SingleItem label="Generator" icon={RectangleEllipsis} href="/generator" />
          ...
          <AccordionItem value='item-1' className="border-b-0">
            ...
            <AccordionContent>
              {children.map(({ item, href, icon: Icon, premium }) => (
                <div key={item} className="flex justify-between items-center hover:bg-blue-100/20 mt-2 pr-1 rounded-md transition-all duration-300">
                  <Link href={href}
      ...
      <SingleItem href="/analytics" icon={BarChart} label="Analytics" />

      <SingleItem onClick={() => console.log('Close session')} href="#" icon={DoorClosed} label="Close sesion" />
    </div>
  )
}
